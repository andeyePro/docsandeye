...
Author the first real Docs&I project: convert the AEP0.2 assembly instructions and bill of materials in the electroPioreactor repository (branch `AEP02`, mounted read-write at `/repos/electroPioreactor`) into a `docs/` tree of components, steps and media manifests plus `docsandeye.config.yaml`, so that `docs.electroPioreactor.org/AEP` can be built from it with the plugin, and so that a sibling `/MEP` guide can later share most of the components and steps. This task writes files only in that repository; it never runs `git add`, `git commit`, `git push` or `git pull` there (T26 pending), and it never edits the existing `Assembly/README.md` or `BoM.md` (T28 pending: replace or coexist is Martin's call).
...
Git in `/repos/electroPioreactor`: read-only commands (`git status`, `git log`, `git diff`, `git rev-parse`) are permitted for the Generator's self-check and the Reviewer's verification; every mutating command (`add`, `commit`, `push`, `pull`, `checkout`, `stash`, `reset`) is barred. The Generator's report records `git -C /repos/electroPioreactor log -1 --format=%H` as its first action of every cycle.
...
Quantity rule                     `parts[].qty` is the number of physical items the step uses: "1 pair" of pumps → the pump component with qty 2 (supplier note "sold as a pair"); "1 per n units", "shared" and "consumable" items (spare vial, spare flea, SodaStream cylinder, Loctite) appear only where a step uses them, with qty 1 and cat consumable, and in step-00's spares list
...
4. Printed parts reference the real `.scad` sources under `Components/` with correct paths (spaces preserved); the render plan from `buildRenderPlan` lists a job for each of them; parameters are left at the source defaults with a `changelog` note that the per-variant values are pending (T27).
...
9. No existing file in the repository is modified and no mutating git command was run against `/repos/electroPioreactor`: the reviewer checks `git -C /repos/electroPioreactor status --short` shows only the pre-existing ` M .gitignore` line plus untracked `docs/` and `docsandeye.config.yaml`, and `git -C /repos/electroPioreactor log -1 --format=%H` equals the hash recorded at the top of the generator report.
...
- Editing `Assembly/README.md`, `BoM.md` or anything else that already exists; committing; pushing.
